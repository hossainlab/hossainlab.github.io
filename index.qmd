...
![](files/research/harmonized-atlas.svg){width=100% style="max-width:100%; display:block; margin:1rem 0;"}
...
<div id="program-toast" class="program-toast no-print">
  ...
  <div class="program-tabs">
    <button class="tab-btn active" onclick="showProgram('bmp')">Research (BMP)</button>
    <button class="tab-btn" onclick="showProgram('nocode')">AI (No-Code)</button>
  </div>
...
function showProgram(id) {
  ...
  var clickedBtn = Array.from(document.querySelectorAll('.tab-btn')).find(b => b.innerText.toLowerCase().includes(id.toLowerCase()));
  if (clickedBtn) clickedBtn.classList.add('active');
  
  // Update content
  document.querySelectorAll('.program-content').forEach(content => content.classList.remove('active'));
  document.getElementById(id + '-content').classList.add('active');
}

(function() {
  if (!localStorage.getItem('program-toast-dismissed')) {
    setTimeout(function() {
      var el = document.getElementById('program-toast');
...
function dismissProgramToast() {
  ...
  localStorage.setItem('program-toast-dismissed', '1');
}
</script>
```
